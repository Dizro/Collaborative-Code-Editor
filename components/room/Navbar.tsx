...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const panels: Panels = {
    invite: {
      ...
      component: <RoomInvite roomId={roomId} isPrivate={isPrivate} />,
    },
    voice: {
      title: "Голосовой чат",
      icon: <FiMic size={20} />,
    ...
    chat: {
      ...
      component: <TextChat username={username} />,
    },
  };
      ...
      <div className="bg-white border-b border-gray-100">
        ...
        <div className="hidden lg:flex px-6 py-4 justify-between items-center">
          <div className="flex items-center space-x-6">
            <Logo />
            <div>
              <h1 className="text-[#323C46] text-lg font-medium mb-1">
                Комната: {roomId}
              </h1>
              <p className="text-sm text-[#9197A0]">
                Участник: <span className="text-[#393B3A]">{username}</span>
              </p>
            </div>
            <button
              onClick={onNewRoom}
              className="px-4 py-2.5 bg-white text-[#323C46] border border-[#9197A0] rounded-xl
                hover:border-[#323C46] hover:bg-gray-50 transition-colors duration-200 text-sm font-medium"
            >
              Новая комната
            </button>
          </div>
          ...
          <div className="flex items-center space-x-4">
            {(Object.entries(panels) as [PanelType, Panel][]).map(
              ([key, panel]) => (
                <button
                  key={key}
                  onClick={() =>
            ...
            <button
              onClick={onLeave}
              className="px-4 py-2.5 text-[#686B75] border border-[#9197A0] rounded-xl
          ...
          <div className="px-4 py-3 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div className="text-sm">
                <h1 className="text-[#323C46] font-medium">
                  Комната: {roomId}
                </h1>
                <p className="text-[#9197A0]">{username}</p>
              </div>
            </div>

            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="p-2 text-[#686B75] hover:bg-gray-50 rounded-xl"
            >
            ...
            {isMobileMenuOpen && (
              ...
              >
                <div className="p-4 space-y-3">
                  <button
                    onClick={onNewRoom}
                    className="w-full px-4 py-2.5 bg-white text-[#323C46] border border-[#9197A0] rounded-xl
                      hover:border-[#323C46] hover:bg-gray-50 transition-colors duration-200 text-sm font-medium"
                  >
                    Новая комната
                  </button>
                    ...
                    {(Object.entries(panels) as [PanelType, Panel][]).map(
                      ([key, panel]) => (
                        <button
                          ...
                          className={`p-3 rounded-xl transition-colors duration-200 flex flex-col items-center
                          ${
                            activePanel === key
                              ? "bg-[#323C46] text-white"
                              : "text-[#686B75] hover:bg-gray-50"
                  ...
                  <button
                    onClick={onLeave}
                    className="w-full px-4 py-2.5 text-[#686B75] border border-[#9197A0] rounded-xl
                      hover:bg-red-50 hover:text-red-600 hover:border-red-200 
                      transition-colors duration-200 text-sm font-medium"
                  >
                    Покинуть комнату
        ...
        {activePanel && (
          <>
            <motion.div
              ...
              className="fixed inset-0 bg-black bg-opacity-20 backdrop-blur-sm z-[998]"
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 h-screen w-full sm:w-[400px] bg-white shadow-xl z-[999] 
                overflow-hidden flex flex-col border-l border-gray-100"
            >
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 className="text-[#323C46] font-medium">
                  {activePanel ? panels[activePanel].title : ""}
                </h3>
                <button
                  onClick={() => setActivePanel(null)}
                  className="p-2 text-[#686B75] hover:bg-gray-50 rounded-xl transition-colors duration-200"
                >
                  <FiX size={20} />
                </button>
              </div>
              <div className="flex-1 overflow-y-auto p-6">
                {activePanel ? panels[activePanel].component : null}
              </div>
            </motion.div>
          </>
        )}
